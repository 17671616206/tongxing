...
//      console.log(data)
        ...
        for(var j= 0;j<data.price.length;j++){
            var address=$('<button class="dress">'+data.price[0].region_name+'</button>');
            $(".placeholder").append(address);

            //购买时长选择

            //转化spec字符串
            var a= eval('(' + data.price[j].spec + ')');
            $(".time").text(a.name);
                for(var h=0;h< a.option.length;h++){
                    $(".accout_middle_time").append($('<div class="time_two">'+a.option[h].value+'</div>'));
                    $(".time_two").click(function(){
                        $(".time_one").addClass("time_two");
                        $(".time_one").removeClass("time_one");
                        $(this).addClass("time_one");
                        $(this).removeClass("time_two");
                        var value=$(this).text();
                        console.log(value);
                        //详细分类具体价格
                        $(".pricename_price").text("￥"+ a.option[value-1].price);
                        $(".servicename_price").text("￥"+ a.option[value-1].integral_price);
                    });
                };
...
//评论模块
$.ajax({
    url:"http://api.tx178178.com/api/GoodsInfo1/evaluate",
    type:"post",
    dataType:"text",
    data:{
        goods_id:1
